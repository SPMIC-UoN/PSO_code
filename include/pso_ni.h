...
typedef struct {
  ...
  double error_ni;
  double *gbest_ni; // should contain DIM elements!!
...
typedef double (*pso_obj_fun_t_ni)(double *, int, void *, double *, double *, double *, double *);
...
typedef struct {
  ...
  int nhood_strategy; // neighborhood strategy (see PSO_niHOOD_*)
...
void pso_solve_ni(pso_obj_fun_t_ni obj_fun_ni, void *obj_fun_params,
               pso_result_t_ni *solution_ni, pso_settings_t_ni *settings_ni, double *specs, double *B1inh, double *MT, double *APT);
