...
int pso_calc_swarm_size_ci(int dim) {
...
double calc_inertia_lin_dec_ci(int step, pso_settings_t_ci *settings_ci) {

  int dec_stage = 3 * settings_ci->steps / 4;
  if (step <= dec_stage)
    return settings_ci->w_min + (settings_ci->w_max - settings_ci->w_min) *  \
      (dec_stage - step) / dec_stage;
  else
    return settings_ci->w_min;
}
...
//==============================================================
// global neighborhood
void inform_global_ci(int *comm, double **pos_nb,
       double *pos_b, double *fit_b,
       double *gbest_ci, int improved,
...
// printf("%d\n", 11111);
  ...
  for (i=0; i<settings_ci->size; i++){

    for (j=0; j<settings_ci->dim; j++){
      pos_nb[i][j]=gbest_ci[j];
      // printf("%f ", pos_nb[i][j]);
    }
    // printf("\n");
...
void inform_ci(int *comm, double *pos_nb, double *pos_b, double *fit_b,
      int improved, pso_settings_t_ci * settings_ci)
{
  int i, j;
  int b_n; // best neighbor in terms of fitness
  // for each particle
  for (j=0; j<settings_ci->size; j++) {
    b_n = j; // self is best
    // who is the best informer??
    for (i=0; i<settings_ci->size; i++)
      // the i^th particle informs the j^th particle
      if (comm[i*settings_ci->size + j] && fit_b[i] < fit_b[b_n])
        // found a better informer for j^th particle
        b_n = i;
    // copy pos_b of b_n^th particle to pos_nb[j]
    memmove((void *)&pos_nb[j*settings_ci->dim],
            (void *)&pos_b[b_n*settings_ci->dim],
            sizeof(double) * settings_ci->dim);
...
void init_comm_ring_ci(int *comm, pso_settings_t_ci * settings_ci) {
  int i;
  // reset array
  memset((void *)comm, 0, sizeof(int)*settings_ci->size*settings_ci->size);

  // choose informers
  for (i=0; i<settings_ci->size; i++) {
    // set diagonal to 1
    comm[i*settings_ci->size+i] = 1;
    if (i==0) {
      // look right
      comm[i*settings_ci->size+i+1] = 1;
      // look left
      comm[(i+1)*settings_ci->size-1] = 1;
    } else if (i==settings_ci->size-1) {
      // look right
      comm[i*settings_ci->size] = 1;
      // look left
      comm[i*settings_ci->size+i-1] = 1;
    } else {
      // look right
...
void inform_ring_ci(int *comm, double *pos_nb,
     double *pos_b, double *fit_b,
     double *gbest_ci, int improved,
     pso_settings_t_ci * settings_ci)
{
  ...
  inform_ci(comm, pos_nb, pos_b, fit_b, improved, settings_ci);
...
void pso_set_default_settings_ci(pso_settings_t_ci *settings_ci) {

  // set some default values
  settings_ci->dim = 4;//30;//22;
  // settings->x_lo = -20;
  // settings->x_hi = 20;
  settings_ci->goal = 1e-5;

  // settings->size = 8538;
  settings_ci->print_every = 10;
  settings_ci->steps = 500;
  settings_ci->c1 = 1.496;
  settings_ci->c2 = 1.496;
  settings_ci->w_max = 0.7298;
  settings_ci->w_min = 0.3;
  // settings->w_max = 0.000000001;
  // settings->w_min = 0.000000001;


  settings_ci->clamp_pos = 1;
  settings_ci->nhood_strategy = PSO_NHOOD_GLOBAL;
  settings_ci->nhood_size = 5;
  settings_ci->w_strategy = PSO_W_LIN_DEC;
...
//                     PSO ALGORITHM
//==============================================================
void pso_solve_ci(pso_obj_fun_t_ci obj_fun_ci, void *obj_fun_params,
         pso_result_t_ci *solution_ci, pso_settings_t_ci *settings_ci, double *specs, double *B1inh, double *MT)
{
  ...
  pos = malloc(settings_ci->size * sizeof(double*));
  vel = malloc(settings_ci->size * sizeof(double*));
  pos_b = malloc(settings_ci->size * sizeof(double*));
  pos_nb = malloc(settings_ci->size * sizeof(double*));


  int X;
  for(X=0;X<settings_ci->size;X++){
  pos[X] = (double *)malloc(settings_ci->dim * sizeof(double));
  vel[X] = (double *)malloc(settings_ci->dim * sizeof(double));
  pos_b[X] = (double *)malloc(settings_ci->dim * sizeof(double));
  pos_nb[X] = (double *)malloc(settings_ci->dim * sizeof(double));
  }

  fit = malloc(settings_ci->size * sizeof(double));
  fit_b = malloc(settings_ci->size * sizeof(double));
  ...
  int* comm;
  comm = malloc(pow(settings_ci->size,2) * sizeof(int));
  ...
  switch (settings_ci->nhood_strategy)
    {
    case PSO_NHOOD_GLOBAL :
      // comm matrix not used
      inform_fun = inform_global_ci;
      break;
    case PSO_NHOOD_RING :
      init_comm_ring_ci((int *)comm, settings_ci);
      inform_fun = inform_ring_ci;
      break;
  ...
  switch (settings_ci->w_strategy)
    {
      /* case PSO_W_CONST : */
      /*     calc_inertia_fun = calc_inertia_const; */
      /*     break; */
    case PSO_W_LIN_DEC :
      calc_inertia_fun = calc_inertia_lin_dec_ci;
      break;
    }

  // INITIALIZE SOLUTION
  solution_ci->error_ci = DBL_MAX;

  // SWARM INITIALIZATION
  // for each particle
  for (i=0; i<settings_ci->size; i++) {
    // for each dimension
    for (d=0; d<settings_ci->dim; d++) {
      ...
      //   gsl_rng_uniform(settings->rng);
      srand(seed++);
      a = settings_ci->x_lo[d] + (settings_ci->x_hi[d] - settings_ci->x_lo[d]) * \
        rand() / 2147483647.0;
      srand(seed++);
      b = settings_ci->x_lo[d] + (settings_ci->x_hi[d] - settings_ci->x_lo[d]) *  \
        rand() / 2147483647.0;;
      // initialize position
      pos[i][d] = a;
    ...
    fit[i] = obj_fun_ci(pos[i], settings_ci->dim, obj_fun_params, specs, B1inh, MT);
    fit_b[i] = fit[i]; // this is also the personal best
    // update gbest??
    if (fit[i] < solution_ci->error_ci) {
      // update best fitness
      solution_ci->error_ci = fit[i];
      // copy particle pos to gbest vector
      int qq;
      for (qq=0; qq<settings_ci->dim; qq++){
        solution_ci->gbest_ci[qq]=pos[i][qq];
  ...
  w = PSO_INERTIA;
  // RUN ALGORITHM
  for (step=0; step<settings_ci->steps; step++) {
    // update current step
    settings_ci->step = step;
    // update inertia weight
    // do not bother with calling a calc_w_const function
    if (settings_ci->w_strategy)
      w = calc_inertia_fun(step, settings_ci);
    // check optimization goal
    if (solution_ci->error_ci <= settings_ci->goal) {
      // SOLVED!!
      if (settings_ci->print_every)
        printf("Goal achieved @ step %d (error=%.3e) :-)\n", step, solution_ci->error_ci);
      break;
    }

    // update pos_nb matrix (find best of neighborhood for all particles)
    inform_fun(comm, pos_nb, pos_b, fit_b, solution_ci->gbest_ci,
               improved, settings_ci);
    ...
    for (i=0; i<settings_ci->size; i++) {
      ...
      for (d=0; d<settings_ci->dim; d++) {
        ...
        // printf("%d\n", 45678);
        srand(seed++);
        rho1 = settings_ci->c1 * rand() / 2147483647.0;
        srand(seed++);
        rho2 = settings_ci->c2 * rand() / 2147483647.0;
        // printf("%d\n", 87654);
        // update velocity
        // printf("%f\n", vel[i][d]);
        // printf("%f\n", pos[i][d]);
        ...
        if (settings_ci->clamp_pos) {
          if (pos[i][d] < settings_ci->x_lo[d]) {
            pos[i][d] = settings_ci->x_lo[d];
            vel[i][d] = 0;
          } else if (pos[i][d] > settings_ci->x_hi[d]) {
            pos[i][d] = settings_ci->x_hi[d];
            vel[i][d] = 0;
          }
        } else {
          ...
          if (pos[i][d] < settings_ci->x_lo[d]) {
            ...
            pos[i][d] = settings_ci->x_hi[d] - fmod(settings_ci->x_lo[d] - pos[i][d],
                                              settings_ci->x_hi[d] - settings_ci->x_lo[d]);
            vel[i][d] = 0;

          } else if (pos[i][d] > settings_ci->x_hi[d]) {

            pos[i][d] = settings_ci->x_lo[d] + fmod(pos[i][d] - settings_ci->x_hi[d],
                                              settings_ci->x_hi[d] - settings_ci->x_lo[d]);
            vel[i][d] = 0;
          }
        }
      ...
      if (fit[i] < solution_ci->error_ci) {
        ...
        // copy particle pos to gbest vector
        int qq1;
        for (qq1=0; qq1<settings_ci->dim; qq1++){
          solution_ci->gbest_ci[qq1]=pos[i][qq1];
        }
    ...
    if (settings_ci->print_every && (step % settings_ci->print_every == 0))
      printf("Step %d (w=%.2f) :: min err=%.10f\n", step, w, solution_ci->error_ci);
