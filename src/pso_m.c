...
int pso_malc_swarm_size(int dim) {
...
double calc_inertia_lin_dec_m(int step, pso_settings_t_m *settings_m) {

  int dec_stage = 3 * settings_m->steps / 4;
  if (step <= dec_stage)
    return settings_m->w_min + (settings_m->w_max - settings_m->w_min) *  \
      (dec_stage - step) / dec_stage;
  else
    return settings_m->w_min;
}
...
//==============================================================
// global neighborhood
void inform_global_m(int *comm, double **pos_nb,
       double *pos_b, double *fit_b,
       double *gbest_m, int improved,
...
// printf("%d\n", 11111);
  ...
  for (i=0; i<settings_m->size; i++){

    for (j=0; j<settings_m->dim; j++){
      pos_nb[i][j]=gbest_m[j];
      // printf("%f ", pos_nb[i][j]);
    }
    // printf("\n");
...
void inform_m(int *comm, double *pos_nb, double *pos_b, double *fit_b,
      int improved, pso_settings_t_m * settings_m)
{
  int i, j;
  int b_n; // best neighbor in terms of fitness
  // for each particle
  for (j=0; j<settings_m->size; j++) {
    b_n = j; // self is best
    // who is the best informer??
    for (i=0; i<settings_m->size; i++)
      // the i^th particle informs the j^th particle
      if (comm[i*settings_m->size + j] && fit_b[i] < fit_b[b_n])
        // found a better informer for j^th particle
        b_n = i;
    // copy pos_b of b_n^th particle to pos_nb[j]
    memmove((void *)&pos_nb[j*settings_m->dim],
            (void *)&pos_b[b_n*settings_m->dim],
            sizeof(double) * settings_m->dim);
...
void init_comm_ring_m(int *comm, pso_settings_t_m * settings_m) {
  int i;
  // reset array
  memset((void *)comm, 0, sizeof(int)*settings_m->size*settings_m->size);

  // choose informers
  for (i=0; i<settings_m->size; i++) {
    // set diagonal to 1
    comm[i*settings_m->size+i] = 1;
    if (i==0) {
      // look right
      comm[i*settings_m->size+i+1] = 1;
      // look left
      comm[(i+1)*settings_m->size-1] = 1;
    } else if (i==settings_m->size-1) {
      // look right
      comm[i*settings_m->size] = 1;
      // look left
      comm[i*settings_m->size+i-1] = 1;
    } else {
      // look right
...
void inform_ring_m(int *comm, double *pos_nb,
     double *pos_b, double *fit_b,
     double *gbest_m, int improved,
     pso_settings_t_m * settings_m)
{
  ...
  inform_m(comm, pos_nb, pos_b, fit_b, improved, settings_m);
...
void pso_set_default_settings_m(pso_settings_t_m *settings_m) {

  // set some default values
  settings_m->dim = 4;//30;//22;
  // settings->x_lo = -20;
  // settings->x_hi = 20;
  settings_m->goal = 1e-5;

  // settings->size = 8538;
  settings_m->print_every = 10;
  settings_m->steps = 500;
  settings_m->c1 = 1.496;
  settings_m->c2 = 1.496;
  settings_m->w_max = 0.7298;
  settings_m->w_min = 0.3;
  // settings->w_max = 0.000000001;
  // settings->w_min = 0.000000001;


  settings_m->clamp_pos = 1;
  settings_m->nhood_strategy = PSO_NHOOD_GLOBAL;
  settings_m->nhood_size = 5;
  settings_m->w_strategy = PSO_W_LIN_DEC;
...
//                     PSO ALGORITHM
//==============================================================
void pso_solve_m(pso_obj_fun_t_m obj_fun_m, void *obj_fun_params,
         pso_result_t_m *solution_m, pso_settings_t_m *settings_m, double *specs, double *B1inh, double *MT, double *APT, double *NOE, double *NOE2i, double *Cri)
{
  ...
  pos = malloc(settings_m->size * sizeof(double*));
  vel = malloc(settings_m->size * sizeof(double*));
  pos_b = malloc(settings_m->size * sizeof(double*));
  pos_nb = malloc(settings_m->size * sizeof(double*));


  int X;
  for(X=0;X<settings_m->size;X++){
  pos[X] = (double *)malloc(settings_m->dim * sizeof(double));
  vel[X] = (double *)malloc(settings_m->dim * sizeof(double));
  pos_b[X] = (double *)malloc(settings_m->dim * sizeof(double));
  pos_nb[X] = (double *)malloc(settings_m->dim * sizeof(double));
  }

  fit = malloc(settings_m->size * sizeof(double));
  fit_b = malloc(settings_m->size * sizeof(double));
  ...
  int* comm;
  comm = malloc(pow(settings_m->size,2) * sizeof(int));
  ...
  switch (settings_m->nhood_strategy)
    {
    case PSO_NHOOD_GLOBAL :
      // comm matrix not used
      inform_fun = inform_global_m;
      break;
    case PSO_NHOOD_RING :
      init_comm_ring_m((int *)comm, settings_m);
      inform_fun = inform_ring_m;
      break;
  ...
  switch (settings_m->w_strategy)
    {
      /* case PSO_W_mONST : */
      /*     calc_inertia_fun = calc_inertia_monst; */
      /*     break; */
    case PSO_W_LIN_DEC :
      calc_inertia_fun = calc_inertia_lin_dec_m;
      break;
    }

  // INITIALIZE SOLUTION
  solution_m->error_m = DBL_MAX;

  // SWARM INITIALIZATION
  // for each particle
  for (i=0; i<settings_m->size; i++) {
    // for each dimension
    for (d=0; d<settings_m->dim; d++) {
      ...
      //   gsl_rng_uniform(settings->rng);
      srand(seed++);
      a = settings_m->x_lo[d] + (settings_m->x_hi[d] - settings_m->x_lo[d]) * \
        rand() / 2147483647.0;
      srand(seed++);
      b = settings_m->x_lo[d] + (settings_m->x_hi[d] - settings_m->x_lo[d]) *  \
        rand() / 2147483647.0;;
      // initialize position
      pos[i][d] = a;
    ...
    fit[i] = obj_fun_m(pos[i], settings_m->dim, obj_fun_params, specs, B1inh, MT, APT, NOE, NOE2i, Cri);
    fit_b[i] = fit[i]; // this is also the personal best
    // update gbest??
    if (fit[i] < solution_m->error_m) {
      // update best fitness
      solution_m->error_m = fit[i];
      // copy particle pos to gbest vector
      int qq;
      for (qq=0; qq<settings_m->dim; qq++){
        solution_m->gbest_m[qq]=pos[i][qq];
  ...
  w = PSO_INERTIA;
  // RUN ALGORITHM
  for (step=0; step<settings_m->steps; step++) {
    // update current step
    settings_m->step = step;
    // update inertia weight
    // do not bother with calling a calc_w_monst function
    if (settings_m->w_strategy)
      w = calc_inertia_fun(step, settings_m);
    // check optimization goal
    if (solution_m->error_m <= settings_m->goal) {
      // SOLVED!!
      if (settings_m->print_every)
        printf("Goal achieved @ step %d (error=%.3e) :-)\n", step, solution_m->error_m);
      break;
    }

    // update pos_nb matrix (find best of neighborhood for all particles)
    inform_fun(comm, pos_nb, pos_b, fit_b, solution_m->gbest_m,
               improved, settings_m);
    ...
    for (i=0; i<settings_m->size; i++) {
      ...
      for (d=0; d<settings_m->dim; d++) {
        ...
        // printf("%d\n", 45678);
        srand(seed++);
        rho1 = settings_m->c1 * rand() / 2147483647.0;
        srand(seed++);
        rho2 = settings_m->c2 * rand() / 2147483647.0;
        // printf("%d\n", 87654);
        // update velocity
        // printf("%f\n", vel[i][d]);
        // printf("%f\n", pos[i][d]);
        ...
        if (settings_m->clamp_pos) {
          if (pos[i][d] < settings_m->x_lo[d]) {
            pos[i][d] = settings_m->x_lo[d];
            vel[i][d] = 0;
          } else if (pos[i][d] > settings_m->x_hi[d]) {
            pos[i][d] = settings_m->x_hi[d];
            vel[i][d] = 0;
          }
        } else {
          ...
          if (pos[i][d] < settings_m->x_lo[d]) {
            ...
            pos[i][d] = settings_m->x_hi[d] - fmod(settings_m->x_lo[d] - pos[i][d],
                                              settings_m->x_hi[d] - settings_m->x_lo[d]);
            vel[i][d] = 0;

          } else if (pos[i][d] > settings_m->x_hi[d]) {

            pos[i][d] = settings_m->x_lo[d] + fmod(pos[i][d] - settings_m->x_hi[d],
                                              settings_m->x_hi[d] - settings_m->x_lo[d]);
            vel[i][d] = 0;
          }
        }
      ...
      if (fit[i] < solution_m->error_m) {
        ...
        // copy particle pos to gbest vector
        int qq1;
        for (qq1=0; qq1<settings_m->dim; qq1++){
          solution_m->gbest_m[qq1]=pos[i][qq1];
        }
    ...
    if (settings_m->print_every && (step % settings_m->print_every == 0))
      printf("Step %d (w=%.2f) :: min err=%.10f\n", step, w, solution_m->error_m);
