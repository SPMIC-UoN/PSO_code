...
double calc_inertia_lin_dec_b(int step, pso_settings_t_b *settings_b) {

  int dec_stage = 3 * settings_b->steps / 4;
  if (step <= dec_stage)
    return settings_b->w_min + (settings_b->w_max - settings_b->w_min) *  \
      (dec_stage - step) / dec_stage;
  else
    return settings_b->w_min;
}
...
//==============================================================
// global neighborhood
void inform_global_b(int *comm, double **pos_nb,
       double *pos_b, double *fit_b,
       double *gbest_b, int improved,
...
// printf("%d\n", 11111);
  ...
  for (i=0; i<settings_b->size; i++){

    for (j=0; j<settings_b->dim; j++){
      pos_nb[i][j]=gbest_b[j];
      // printf("%f ", pos_nb[i][j]);
    }
    // printf("\n");
...
void inform_b(int *comm, double *pos_nb, double *pos_b, double *fit_b,
      int improved, pso_settings_t_b * settings_b)
{
  int i, j;
  int b_n; // best neighbor in terms of fitness
  // for each particle
  for (j=0; j<settings_b->size; j++) {
    b_n = j; // self is best
    // who is the best informer??
    for (i=0; i<settings_b->size; i++)
      // the i^th particle informs the j^th particle
      if (comm[i*settings_b->size + j] && fit_b[i] < fit_b[b_n])
        // found a better informer for j^th particle
        b_n = i;
    // copy pos_b of b_n^th particle to pos_nb[j]
    memmove((void *)&pos_nb[j*settings_b->dim],
            (void *)&pos_b[b_n*settings_b->dim],
            sizeof(double) * settings_b->dim);
...
void init_comm_ring_b(int *comm, pso_settings_t_b * settings_b) {
  int i;
  // reset array
  memset((void *)comm, 0, sizeof(int)*settings_b->size*settings_b->size);

  // choose informers
  for (i=0; i<settings_b->size; i++) {
    // set diagonal to 1
    comm[i*settings_b->size+i] = 1;
    if (i==0) {
      // look right
      comm[i*settings_b->size+i+1] = 1;
      // look left
      comm[(i+1)*settings_b->size-1] = 1;
    } else if (i==settings_b->size-1) {
      // look right
      comm[i*settings_b->size] = 1;
      // look left
      comm[i*settings_b->size+i-1] = 1;
    } else {
      // look right
...
void inform_ring_b(int *comm, double *pos_nb,
     double *pos_b, double *fit_b,
     double *gbest_b, int improved,
     pso_settings_t_b * settings_b)
{
  ...
  inform_b(comm, pos_nb, pos_b, fit_b, improved, settings_b);
...
//   inform_b(comm, pos_nb, pos_b, fit_b, improved, settings);
...
void pso_set_default_settings_b(pso_settings_t_b *settings_b) {
  ...
  settings_b->dim = 5;//30;//22;
  // settings->x_lo = -20;
  // settings->x_hi = 20;
  settings_b->goal = 1e-5;

  // settings->size = 8538;
  settings_b->print_every = 10;
  settings_b->steps = 500;
  settings_b->c1 = 1.496;
  settings_b->c2 = 1.496;
  settings_b->w_max = 0.7298;
  settings_b->w_min = 0.3;
  // settings->w_max = 0.000000001;
  // settings->w_min = 0.000000001;


  settings_b->clamp_pos = 1;
  settings_b->nhood_strategy = PSO_NHOOD_GLOBAL;
  settings_b->nhood_size = 5;
  settings_b->w_strategy = PSO_W_LIN_DEC;
         ...
         pso_result_t_b *solution_b, pso_settings_t_b *settings_b, double *specs, double *B1inh, double *MTi, double *APTi, double *NOEi, double *NOE2i, double *Cri)
{
  ...
  pos = malloc(settings_b->size * sizeof(double*));
  vel = malloc(settings_b->size * sizeof(double*));
  pos_b = malloc(settings_b->size * sizeof(double*));
  pos_nb = malloc(settings_b->size * sizeof(double*));


  int X;
  for(X=0;X<settings_b->size;X++){
  pos[X] = (double *)malloc(settings_b->dim * sizeof(double));
  vel[X] = (double *)malloc(settings_b->dim * sizeof(double));
  pos_b[X] = (double *)malloc(settings_b->dim * sizeof(double));
  pos_nb[X] = (double *)malloc(settings_b->dim * sizeof(double));
  }

  fit = malloc(settings_b->size * sizeof(double));
  fit_b = malloc(settings_b->size * sizeof(double));
  ...
  int* comm;
  comm = malloc(pow(settings_b->size,2) * sizeof(int));
  ...
  switch (settings_b->nhood_strategy)
    {
    case PSO_NHOOD_GLOBAL :
      // comm matrix not used
      inform_fun = inform_global_b;
      break;
    case PSO_NHOOD_RING :
      init_comm_ring_b((int *)comm, settings_b);
      inform_fun = inform_ring_b;
      break;
  ...
  switch (settings_b->w_strategy)
    {
      /* case PSO_W_CONST : */
      /*     calc_inertia_fun = calc_inertia_const; */
      /*     break; */
    case PSO_W_LIN_DEC :
      calc_inertia_fun = calc_inertia_lin_dec_b;
      break;
    }

  // INITIALIZE SOLUTION
  solution_b->error_b = DBL_MAX;

  // SWARM INITIALIZATION
  // for each particle
  for (i=0; i<settings_b->size; i++) {
    // for each dimension
    for (d=0; d<settings_b->dim; d++) {
      ...
      //   gsl_rng_uniform(settings->rng);
      srand(seed++);
      a = settings_b->x_lo[d] + (settings_b->x_hi[d] - settings_b->x_lo[d]) * \
        rand() / 2147483647.0;
      srand(seed++);
      b = settings_b->x_lo[d] + (settings_b->x_hi[d] - settings_b->x_lo[d]) *  \
        rand() / 2147483647.0;;
      // initialize position
      pos[i][d] = a;
    ...
    fit[i] = obj_fun_b(pos[i], settings_b->dim, obj_fun_params, specs, B1inh, MTi, APTi, NOEi, NOE2i, Cri);
    fit_b[i] = fit[i]; // this is also the personal best
    // update gbest??
    if (fit[i] < solution_b->error_b) {
      // update best fitness
      solution_b->error_b = fit[i];
      // copy particle pos to gbest vector
      int qq;
      for (qq=0; qq<settings_b->dim; qq++){
        solution_b->gbest_b[qq]=pos[i][qq];
  ...
  w = PSO_INERTIA;
  // RUN ALGORITHM
  for (step=0; step<settings_b->steps; step++) {
    // update current step
    settings_b->step = step;
    // update inertia weight
    // do not bother with calling a calc_w_const function
    if (settings_b->w_strategy)
      w = calc_inertia_fun(step, settings_b);
    // check optimization goal
    if (solution_b->error_b <= settings_b->goal) {
      // SOLVED!!
      if (settings_b->print_every)
        printf("Goal achieved @ step %d (error=%.3e) :-)\n", step, solution_b->error_b);
      break;
    }

    // update pos_nb matrix (find best of neighborhood for all particles)
    inform_fun(comm, pos_nb, pos_b, fit_b, solution_b->gbest_b,
               improved, settings_b);
    ...
    for (i=0; i<settings_b->size; i++) {
      ...
      for (d=0; d<settings_b->dim; d++) {
        ...
        // printf("%d\n", 45678);
        srand(seed++);
        rho1 = settings_b->c1 * rand() / 2147483647.0;
        srand(seed++);
        rho2 = settings_b->c2 * rand() / 2147483647.0;
        // printf("%d\n", 87654);
        // update velocity
        // printf("%f\n", vel[i][d]);
        // printf("%f\n", pos[i][d]);
        ...
        if (settings_b->clamp_pos) {
          if (pos[i][d] < settings_b->x_lo[d]) {
            pos[i][d] = settings_b->x_lo[d];
            vel[i][d] = 0;
          } else if (pos[i][d] > settings_b->x_hi[d]) {
            pos[i][d] = settings_b->x_hi[d];
            vel[i][d] = 0;
          }
        } else {
          ...
          if (pos[i][d] < settings_b->x_lo[d]) {
            ...
            pos[i][d] = settings_b->x_hi[d] - fmod(settings_b->x_lo[d] - pos[i][d],
                                              settings_b->x_hi[d] - settings_b->x_lo[d]);
            vel[i][d] = 0;

          } else if (pos[i][d] > settings_b->x_hi[d]) {

            pos[i][d] = settings_b->x_lo[d] + fmod(pos[i][d] - settings_b->x_hi[d],
                                              settings_b->x_hi[d] - settings_b->x_lo[d]);
            vel[i][d] = 0;
          }
        }
      ...
      if (fit[i] < solution_b->error_b) {
        ...
        // copy particle pos to gbest vector
        int qq1;
        for (qq1=0; qq1<settings_b->dim; qq1++){
          solution_b->gbest_b[qq1]=pos[i][qq1];
        }
    ...
    if (settings_b->print_every && (step % settings_b->print_every == 0))
      printf("Step %d (w=%.2f) :: min err=%.10f\n", step, w, solution_b->error_b);
